#!/usr/bin/env python3
"""
Batch MoMentS evaluation for Emotion-LLaMA.

CUDA_VISIBLE_DEVICES=4 nice -n 15 taskset -c 0-6 \
  python /scratch/monroy/Playground/EVALUATE_MOMENTS3.py \
  --video-dir /scratch/monroy/Playground/datasets/grounded_videos \
  --out-dir /scratch/monroy/Playground/EXPERIMENTS_RERUN_FINAL/grounded_script3_00 \
...
CUDA_VISIBLE_DEVICES=4 nice -n 15 taskset -c 0-6 \
  ...
  --out-dir /scratch/monroy/Playground/EXPERIMENTS_CLEAN_t0_greedy_backup/CHECK_1506 \
...
CUDA_VISIBLE_DEVICES=1 nice -n 15 taskset -c 0-6 \
  ...
  --out-dir /scratch/monroy/Playground/Experiments_Baseline_RERUN/audio_only
...
CUDA_VISIBLE_DEVICES=1 nice -n 15 taskset -c 0-6 \
  ...
  --out-dir /scratch/monroy/Playground/Experiments_Baseline_RERUN/video_only
...
CUDA_VISIBLE_DEVICES=1 nice -n 15 taskset -c 0-6 \
  ...
  --out-dir /scratch/monroy/Playground/Experiments_YOLO_RERUN/yolov11mface_video2video_audio_2
  """
...
DEFAULT_VIDEO_ROOT = "/scratch/monroy/Playground"
DEFAULT_OUT_ROOT = "/scratch/monroy/Playground"
...
DEFAULT_SEED = 42
DEFAULT_TEMPERATURE = 0.0  # greedy decoding (do_sample=False); deterministic and order-independent

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate Emotion-LLaMA on one or more MoMentS video folders."
    )
    parser.add_argument(
        "--video-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Sampling temperature. Default 0 = greedy decoding (deterministic). Values > 0 enable sampling.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def load_official_model(cfg_path: str = DEFAULT_CFG_PATH, gpu_id: int = 0):
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "CUDA_VISIBLE_DEVICES=<GPU_ID> conda run -n llama-lola python EVALUATE_MOMENTS3.py"
        )
...
def main():
    ...
    if len(all_metrics) > 1:
        summary_path = args.out_root / f"moments1_batch_summary_{stamp}.json"
