#!/usr/bin/env python3
"""
Batch MoMentS evaluation for Emotion-LLaMA — with transcript support.
Based on EVALUATE_MOMENTS3.py, adds transcript injection into the prompt.
...
CUDA_VISIBLE_DEVICES=<GPU> nice -n 15 taskset -c 0-6 \
  python /scratch/monroy/Playground/EVALUATE_MOMENTS3_T.py \
  --video-dir /scratch/monroy/Playground/Experiments_Baseline/audio_only/dataset_audio_only \
  --out-dir /scratch/monroy/Playground/Experiments_Baseline_RERUN/audio_only_transcript \
...
def evaluate_video_dir(
    ...
    for clip_path in video_files:
        ...
        for qrec in qrecs:
            ...
            video_id = str(qrec.get("video_id", "")).strip()
            transcript = transcript_map.get(video_id, "") if use_transcript else ""
