#!/usr/bin/env python3
"""
REFERENT_ANCHOR_FACE_v2.py  —  improved version of REFERENT_ANCHOR_FACE.py
Original logic is unchanged; the following issues are fixed/improved:

  FIX 1  build_candidate_sheet_for_frame: bbox coordinates were drawn at
          original-frame resolution on a downscaled canvas (768px cap), so
          Qwen was seeing misplaced / off-screen boxes.  Now the coordinates
          are scaled together with the canvas.

  FIX 2  detect_people_all_frames: YOLO was called once per frame.  It now
          processes frames in mini-batches (default 16), which is typically
          4-8× faster on GPU because the model runs once per batch instead
          of once per frame.

  FIX 3  propagate_anchor_identity_face_strict: the anchor embedding was
          static for the whole video.  It now performs a slow exponential
          moving-average update from matches whose cosine similarity with the
          *original* anchor is above ANCHOR_UPDATE_THRESHOLD.  This keeps the
          tracker stable through pose and lighting changes without drifting to
          the wrong person.

  FIX 4  tqdm progress bars in detect_people_all_frames and the dataset-mode
          loop (gracefully falls back to plain iteration if tqdm is not
          installed).
...
# This server's libcudnn_graph.so.9 is missing the cudnnGetLibConfig symbol.
# Disabling cuDNN globally makes PyTorch fall back to its own CUDA kernels for
# all models (YOLO, Qwen, InsightFace).  Everything still runs on GPU.
...
# How confident a match must be (cosine sim vs original anchor) before its
# embedding contributes to the rolling anchor update.
ANCHOR_UPDATE_THRESHOLD = 0.65
# EMA weight given to each new high-confidence embedding.
...
# FIX 2: batch inference instead of frame-by-frame.
# cuDNN is disabled around YOLO calls because some servers have a cuDNN build
# that is missing the cudnnGetLibConfig symbol, which YOLO's batched forward
# pass triggers.  Disabling cuDNN makes PyTorch use its own CUDA kernels
# instead — still on GPU, just without the broken cuDNN graph path.
def detect_people_all_frames(
    ...
    batches = range(0, len(frames_bgr), batch_size)
    for start in tqdm(batches, desc="YOLO detection", unit="batch"):
...
# FIX 1: scale bbox coordinates together with the canvas.
def build_candidate_sheet_for_frame(
    ...
    # Track the scale so we can project original bbox coords onto the canvas.
    scale = 1.0
    if max(h, w) > 768:
        scale = 768.0 / max(h, w)
    ...
    for cid, det in candidate_map.items():
        # Apply the same scale used on the canvas.
...
def choose_anchor_frame_and_box(
    ...
    anchor_frame_start: Optional[int] = None,
    anchor_frame_end: Optional[int] = None,
) -> Dict[str, Any]:
    qwen_model, qwen_processor = load_qwen(qwen_model_name)
    lo = anchor_frame_start if anchor_frame_start is not None else 0
    hi = anchor_frame_end if anchor_frame_end is not None else len(frames_bgr) - 1
    lo = max(0, min(lo, len(frames_bgr) - 1))
    hi = max(lo, min(hi, len(frames_bgr) - 1))
    pool = list(range(lo, hi + 1))
    raw_indices = sample_frame_indices(len(pool), n_anchor_frames)
    frame_indices = [pool[i] for i in raw_indices]
    scored = []
...
# FIX 3: rolling anchor embedding update.
def propagate_anchor_identity_face_strict(
    ...
    # Rolling embedding starts as the original anchor.
    # It updates gradually from high-confidence matches so the tracker
    # adapts to small pose/lighting changes without drifting to a wrong person.
    rolling_emb = anchor_face_emb.copy()

    def best_face_match(
        fi: int,
        prev_center: Tuple[float, float],
        current_emb: np.ndarray,
    ) -> Tuple[Optional[Tuple], Optional[Tuple[float, float]], Optional[np.ndarray]]:
        dets = all_dets[fi]
        best_score = -1e9
        best_box = None
        best_center = None
        ...
        for det in dets:
            ...
            face_sim = cosine_sim(current_emb, det_face_emb)
            ...
            if score > best_score:
                ...
                best_box = det["bbox"]
                best_center = (cx, cy)
                best_emb = det_face_emb

        return best_box, best_center, best_emb

    def maybe_update_rolling(emb: np.ndarray, matched_emb: np.ndarray) -> np.ndarray:
        # Only incorporate matches that are still very close to the original
        # anchor to prevent accumulating drift.
        if cosine_sim(anchor_face_emb, matched_emb) >= ANCHOR_UPDATE_THRESHOLD:
            updated = (1.0 - ANCHOR_EMA_ALPHA) * emb + ANCHOR_EMA_ALPHA * matched_emb
    ...
    for fi in range(anchor_frame_idx + 1, len(frames_bgr)):
        best_box, best_center, best_emb = best_face_match(fi, prev_center, rolling_emb)
        ...
        if best_box is not None:
            ...
            hold_count = 0
            rolling_emb = maybe_update_rolling(rolling_emb, best_emb)
...
def parse_args():
    ...
    ap.add_argument("--yolo-batch-size", type=int, default=16,
                    help="Number of frames per YOLO inference batch (default: 16)")
    ap.add_argument("--anchor-t-start", type=float, default=None,
                    help="Restrict anchor frame sampling to frames at or after this timestamp (seconds)")
    ap.add_argument("--anchor-t-end", type=float, default=None,
                    help="Restrict anchor frame sampling to frames at or before this timestamp (seconds)")
...
def run_one_sample(
    ...
    all_dets = detect_people_all_frames(
        ...
        conf=args.conf,
        device=args.device,
        batch_size=getattr(args, "yolo_batch_size", 16),
    )

    print("[3/6] Loading face model and choosing anchor frame + candidate with Qwen")
    models = get_face_models(args.device)
    t_start = getattr(args, "anchor_t_start", None)
    t_end   = getattr(args, "anchor_t_end", None)
    af_start = int(t_start * fps) if t_start is not None else None
    af_end   = int(t_end   * fps) if t_end   is not None else None
    anchor_result = choose_anchor_frame_and_box(
        ...
        anchor_frame_start=af_start,
        anchor_frame_end=af_end,
    )
...
def main():
    ...
    if args.dataset_json is not None:
        ...
        # FIX 4: tqdm on the dataset loop so you can see progress across samples.
        with open(success_path, "a", encoding="utf-8") as f_ok, open(failed_path, "a", encoding="utf-8") as f_fail:
            for idx, sample in enumerate(tqdm(samples, desc="Dataset samples")):
                try:
